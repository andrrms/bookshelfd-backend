...
export default class Logger {
	 ...
	 */
	private static dateString(date?: Date | string, withDate: boolean = true): string {
		let targetDate: Date;
	...
	static verbose(message: string): void {
		...
		if (Logger.canLog('verbose'))
			console.log(`${pc.cyan('[VERBOSE]')} ${pc.blue(Logger.dateString(undefined, false))} - ${message}`);
	}
	...
	static info(message: string): void {
		...
		if (Logger.canLog('info'))
			console.log(`${pc.green('[LOG]')} ${pc.blue(Logger.dateString(undefined, false))} - ${message}`);
	}

	static error(message: string): void {
		Logger.writeToFile(message, 'error');
		if (Logger.canLog('error'))
			console.error(`${pc.red('[ERROR]')} ${pc.blue(Logger.dateString(undefined, false))} - ${message}`);
	}
}
